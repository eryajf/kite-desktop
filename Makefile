...
.PHONY: help dev build clean test docker-build docker-run frontend backend install deps desktop-build desktop-dev e2e-install e2e-kind-up e2e-kind-down e2e-stop-app e2e-setup-ldap e2e-setup-dex e2e-test e2e-test-headed
...
GO_SOURCES := $(shell find . -name '*.go' -not -path './ui/*' -not -path './docs/*' -not -path './e2e/*')
...
$(BINARY_NAME): $(GO_SOURCES) go.mod static
