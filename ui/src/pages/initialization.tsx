...
import { isDesktopMode, openNativeFile } from '@/lib/desktop'
...
export function InitializationPage() {
  ...
  const handleNativeFileSelect = async () => {
    try {
      const selection = await openNativeFile({
        title: t('initialization.step2.nativeFileTitle', {
          defaultValue: '选择 kubeconfig 文件',
        }),
        buttonText: t('initialization.step2.nativeFileButton', {
          defaultValue: '选择文件',
        }),
        message: t('initialization.step2.nativeFileHint', {
          defaultValue: '请选择要导入的 kubeconfig 文件',
        }),
        readContent: true,
      })
      ...
      setSelectedFileName(selection.name || selection.path || null)
      setKubeconfig(selection.content || '')
      toast.success(
        t('initialization.step2.nativeFileSelected', {
          defaultValue: '已选择文件：{{name}}',
          name: selection.name || selection.path || 'kubeconfig',
        })
      )
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : t('initialization.step2.nativeFileError', {
              defaultValue: '打开原生文件选择器失败',
            })
      setError(message)
      toast.error(message)
    }
                      ...
                      <div className="space-y-2">
                        {desktopMode ? (
                          <Button
                            type="button"
                            variant="outline"
                            onClick={() => void handleNativeFileSelect()}
                          >
                            {t('initialization.step2.nativeFileButton', {
                              defaultValue: '选择文件',
                            })}
                          </Button>
                        ) : (
                        ...
                        {selectedFileName ? (
                          <p className="text-sm text-gray-600">
                            {t('initialization.step2.selectedFile', {
                              defaultValue: '已选择：{{name}}',
